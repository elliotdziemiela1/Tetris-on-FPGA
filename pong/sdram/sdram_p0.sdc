...
#      in the sdram_p0_timing.tcl script.
#    * The helper routines are defined in sdram_p0_pin_map.tcl
#
# NOTE
# ----
...
source "$script_dir/sdram_p0_timing.tcl"
...
set io_standard "$::GLOBAL_sdram_p0_io_standard CLASS I"
...
set half_period [ sdram_p0_round_3dp [ expr $t(CK) / 2.0 ] ]

# Half of reference clock
set ref_half_period [ sdram_p0_round_3dp [ expr $t(refCK) / 2.0 ] ]
...
set data_output_min_delay [ sdram_p0_round_3dp [ expr - $t(wru_output_min_delay_external) - $t(wru_output_min_delay_internal)]]
...
set data_output_max_delay [ sdram_p0_round_3dp [ expr $t(wru_output_max_delay_external) + $t(wru_output_max_delay_internal)]]
...
set ac_min_delay [ sdram_p0_round_3dp [ expr - $t(ac_output_min_delay_external) - $t(ac_output_min_delay_internal)]]
...
set ac_max_delay [ sdram_p0_round_3dp [ expr $t(ac_output_max_delay_external) + $t(ac_output_max_delay_internal)]]
...
if { ! [ info exists sdram_p0_sdc_cache ] } {
	set sdram_p0_sdc_cache 1
	sdram_p0_initialize_ddr_db sdram_p0_ddr_db
} else {
	if { $debug } {
...
# following loop
set instances [ array names sdram_p0_ddr_db ]
foreach { inst } $instances {
	if { [ info exists pins ] } {
		# Clean-up stale content
		unset pins
	}
	array set pins $sdram_p0_ddr_db($inst)
	...
	# AFI clock
	set local_pll_afi_clk [ sdram_p0_get_or_add_clock_vseries \
		-target $pll_afi_clock \
		-suffix "afi_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_sdram_p0_pll_mult(PLL_AFI_CLK) \
		-divide_by $::GLOBAL_sdram_p0_pll_div(PLL_AFI_CLK) \
		-phase $::GLOBAL_sdram_p0_pll_phase(PLL_AFI_CLK) ]
	
	# DQ write clock
	set local_pll_dq_write_clk [ sdram_p0_get_or_add_clock_vseries \
		-target $pll_dq_write_clock \
		-suffix "dq_write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_sdram_p0_pll_mult(PLL_WRITE_CLK) \
		-divide_by $::GLOBAL_sdram_p0_pll_div(PLL_WRITE_CLK) \
		-phase $::GLOBAL_sdram_p0_pll_phase(PLL_WRITE_CLK) ]
	
	# DQS write clock
	set local_pll_write_clk [ sdram_p0_get_or_add_clock_vseries \
		-target $pll_write_clock \
		-suffix "write_clk" \
		-source $pll_ref_clock \
		-multiply_by $::GLOBAL_sdram_p0_pll_mult(PLL_MEM_CLK) \
		-divide_by $::GLOBAL_sdram_p0_pll_div(PLL_MEM_CLK) \
		-phase $::GLOBAL_sdram_p0_pll_phase(PLL_MEM_CLK) ]
	...
	set resync_clock ${inst}|pll0|upll_memphy|auto_generated|pll1|clk\[2\]
        ...
        set local_resync_clk [ sdram_p0_get_or_add_clock_vseries \
  		-target $resync_clock \
  		-suffix "resync_clk" \
  		-source $pll_ref_clock \
  		-multiply_by $::GLOBAL_sdram_p0_pll_mult(PLL_ADDR_CMD_CLK) \
  		-divide_by $::GLOBAL_sdram_p0_pll_div(PLL_ADDR_CMD_CLK) \
  		-phase $::GLOBAL_sdram_p0_pll_phase(PLL_ADDR_CMD_CLK) ]


	set tracking_clock ${inst}|pll0|upll_memphy|auto_generated|pll1|clk\[3\]
  	
        set local_tracking_clk [ sdram_p0_get_or_add_clock_vseries \
  		-target $tracking_clock \
  		-suffix "tracking_clk" \
  		-source $pll_ref_clock \
  		-multiply_by $::GLOBAL_sdram_p0_pll_mult(PLL_AFI_HALF_CLK) \
  		-divide_by $::GLOBAL_sdram_p0_pll_div(PLL_AFI_HALF_CLK) \
  		-phase $::GLOBAL_sdram_p0_pll_phase(PLL_AFI_HALF_CLK) ]
	...
	foreach dqsn_out_clock_struct $dqsn_out_clocks {
        ...
        foreach { ck_pin } $ck_pins { 
        	set_output_delay -add_delay -clock [get_clocks $ck_pin] -max [sdram_p0_round_3dp [expr $t(CK) - $t(DQSS)*$t(CK) - $board(minCK_DQS_skew) ]] $dqs_pins
        	set_output_delay -add_delay -clock [get_clocks $ck_pin] -min [sdram_p0_round_3dp [expr $t(DQSS)*$t(CK) - $board(maxCK_DQS_skew) ]] $dqs_pins
...
add_ddr_report_command "source [list [file join [file dirname [info script]] ${::GLOBAL_sdram_p0_corename}_report_timing.tcl]]"
