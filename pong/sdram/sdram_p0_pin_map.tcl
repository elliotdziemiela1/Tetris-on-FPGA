...
# sdram_p0_pin_assignments.tcl and sdram_p0.sdc scripts. 
#
...
source [file join $script_dir sdram_p0_parameters.tcl]
load_package sdc_ext

proc sdram_p0_find_all_pins { mystring } {
...
proc sdram_p0_index_in_collection { col j } {
	set i 0
...
proc sdram_p0_get_clock_to_pin_name_mapping {} {
	set result [list]
	...
	foreach_in_collection clock $clocks_collection { 
		...
		set first_index [sdram_p0_index_in_collection $clock_target 0]
...
proc sdram_p0_get_clock_name_from_pin_name { pin_name } {
	set table [sdram_p0_get_clock_to_pin_name_mapping]
	foreach entry $table {
...
proc sdram_p0_get_clock_name_from_pin_name_vseries {pin_name suffix} {
	set name [sdram_p0_get_clock_name_from_pin_name $pin_name]
...
proc sdram_p0_get_clock_name_from_pin_name_pre_vseries {pin_name suffix} {
	set name [sdram_p0_get_clock_name_from_pin_name $pin_name]
	if {[string compare -nocase $name ""] == 0} {
		set pll_clock $pin_name
...
proc sdram_p0_get_or_add_clock_vseries_from_virtual_refclk {args} {
	array set opts { /
		-suffix "" /
		-target "" /
		-period "" /
	...
	set clock_name [sdram_p0_get_clock_name_from_pin_name $opts(-target)]
...
proc sdram_p0_get_or_add_clock_vseries {args} {
	array set opts { /
		-suffix "" /
		-target "" /
		-source "" /
...
proc sdram_p0_get_or_add_clock_pre_vseries {args} {
	array set opts { /
		-suffix "" /
		-target "" /
		-source "" /
...
proc sdram_p0_get_source_clock_pin_name {node_name} {

	set nodename ""
	set nodes [get_nodes $node_name]
	sdram_p0_traverse_fanin_up_to_depth [sdram_p0_index_in_collection $nodes 0] sdram_p0_is_node_type_pll_clk clock results_array 10
...
proc sdram_p0_find_all_keepers { mystring } {
...
proc sdram_p0_round_3dp { x } {
	return [expr { round($x * 1000) / 1000.0  } ]
}

proc sdram_p0_get_timequest_name {hier_name} {
	set sta_name ""
...
proc sdram_p0_are_entity_names_on { } {
...
proc sdram_p0_get_core_instance_list {corename} {
	set full_instance_list [sdram_p0_get_core_full_instance_list $corename]
	set instance_list [list]

	foreach inst $full_instance_list {
		set sta_name [sdram_p0_get_timequest_name $inst]
...
proc sdram_p0_get_core_full_instance_list {corename} {
...
proc sdram_p0_traverse_fanin_up_to_depth { node_id match_command edge_type results_array_name depth} {
	...
	for {set i 0} {$i != $number_of_fanin_edges} {incr i} {
		...
		if {$match_command == "" || [eval $match_command $fanin_id] != 0} {
			...
			sdram_p0_traverse_fanin_up_to_depth $fanin_id $match_command $edge_type results [expr {$depth - 1}]
		}
	}
}
proc sdram_p0_is_node_type_pll_inclk { node_id } {
...
proc sdram_p0_is_node_type_pin { node_id } {
...
proc sdram_p0_get_input_clk_id { pll_output_node_id } {
	if {[sdram_p0_is_node_type_pll_clk $pll_output_node_id]} {
		array set results_array [list]
		sdram_p0_traverse_fanin_up_to_depth $pll_output_node_id sdram_p0_is_node_type_pll_inclk clock results_array 1
		if {[array size results_array] == 1} {
			...
			sdram_p0_traverse_fanin_up_to_depth $pll_inclk_id sdram_p0_is_node_type_pin clock results_array 3
			if {[array size results_array] == 1} {
				# Fed by a dedicated input pin
				set pin_id [lindex [array names results_array] 0]
				set result $pin_id
			} else {
				sdram_p0_traverse_fanin_up_to_depth $pll_inclk_id sdram_p0_is_node_type_pll_clk clock pll_clk_results_array 1
				sdram_p0_traverse_fanin_up_to_depth $pll_inclk_id sdram_p0_is_node_type_pll_clk clock pll_clk_results_array2 2
				if {[array size pll_clk_results_array] == 1} {
					...
					set result [sdram_p0_get_input_clk_id $source_pll_clk_id]
					if {$result != -1} {
					...
					sdram_p0_traverse_fanin_up_to_depth $pll_inclk_id sdram_p0_is_node_type_pin clock results_array 20
					if {[array size results_array] == 1} {
						set pin_id [lindex [array names results_array] 0]
						set result $pin_id
					} else {
		...
		error "Internal error: sdram_p0_get_input_clk_id only works on PLL output clocks"
	}
	return $result
}

proc sdram_p0_is_node_type_pll_clk { node_id } {
	set cell_id [get_node_info -cell $node_id]
...
proc sdram_p0_get_pll_clock { dest_id_list node_type clock_id_name search_depth} {
	if {$clock_id_name != ""} {
	...
	foreach node_id $dest_id_list {
		sdram_p0_traverse_fanin_up_to_depth $node_id sdram_p0_is_node_type_pll_clk clock clk_array $search_depth
	}
...
proc sdram_p0_get_pll_clock_name { clock_id } {
	set clock_name [get_node_info -name $clock_id]

	return $clock_name
}

proc sdram_p0_get_pll_clock_name_for_acf { clock_id pll_output_wire_name } {
	set clock_name [get_node_info -name $clock_id]
...
proc sdram_p0_get_output_clock_id { ddio_output_pin_list pin_type msg_list_name {max_search_depth 13} } {
	...
	sdram_p0_get_pll_clock $output_id_list $pin_type output_clock_id $max_search_depth
	return $output_clock_id
}

proc sdram_p0_get_output_clock_id2 { ddio_output_pin_list pin_type msg_list_name {max_search_depth 20} } {
	upvar 1 $msg_list_name msg_list
	set output_clock_id -1
...
proc sdram_p0_is_node_type_clkbuf { node_id } {
	set cell_id [get_node_info -cell $node_id]
	if {$cell_id == ""} {
		set result 0
	} else {
...
proc sdram_p0_get_clkbuf_clock { dest_id_list node_type clock_id_name search_depth} {
	if {$clock_id_name != ""} {
		upvar 1 $clock_id_name clock_id
	}
	set clock_id -1
	...
	array set clk_array [list]
	foreach node_id $dest_id_list {
		sdram_p0_traverse_fanin_up_to_depth $node_id sdram_p0_is_node_type_clkbuf clock clk_array $search_depth
	}
	if {[array size clk_array] == 1} {
		set clock_id [lindex [array names clk_array] 0]
		set clk [get_node_info -name $clock_id]
...
proc sdram_p0_get_output_clock_clkbuf_id { ddio_output_pin_list pin_type msg_list_name {max_search_depth 13} } {
	upvar 1 $msg_list_name msg_list
	set output_clock_id -1
	...
	sdram_p0_get_clkbuf_clock $output_id_list $pin_type output_clock_id $max_search_depth
	return $output_clock_id
}


proc sdram_p0_is_node_type_clk_phase_select { node_id } {
	set cell_id [get_node_info -cell $node_id]
	if {$cell_id == ""} {
		set result 0
	} else {
...
proc sdram_p0_get_clk_phase_select_clock { dest_id_list node_type clock_id_name search_depth} {
	if {$clock_id_name != ""} {
		upvar 1 $clock_id_name clock_id
	}
	set clock_id -1
	...
	array set clk_array [list]
	foreach node_id $dest_id_list {
		sdram_p0_traverse_fanin_up_to_depth $node_id sdram_p0_is_node_type_clk_phase_select clock clk_array $search_depth
	}
	if {[array size clk_array] == 1} {
		set clock_id [lindex [array names clk_array] 0]
		set clk [get_node_info -name $clock_id]
...
proc sdram_p0_get_output_clock_clk_phase_select_id { ddio_output_pin_list pin_type msg_list_name {max_search_depth 20} } {
	upvar 1 $msg_list_name msg_list
	set output_clock_id -1
	...
	sdram_p0_get_clk_phase_select_clock $output_id_list $pin_type output_clock_id $max_search_depth
	return $output_clock_id
}
...
proc sdram_p0_get_names_in_collection { col } {
	set res [list]
...
proc sdram_p0_static_map_expand_list { FH listname pinname } {
...
proc sdram_p0_static_map_expand_list_of_list { FH listname pinname } {
	upvar $listname local_list
...
proc sdram_p0_static_map_expand_string { FH stringname pinname } {
...
proc sdram_p0_format_3dp { x } {
	return [format %.3f $x]
}

proc sdram_p0_get_colours { x y } {
...
proc sdram_p0_max_in_collection { col attribute } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {$i == 0} {
...
proc sdram_p0_min_in_collection { col attribute } {
	set i 0
	set min 0
	foreach_in_collection path $col {
		if {$i == 0} {
...
proc sdram_p0_min_in_collection_to_name { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc sdram_p0_min_in_collection_from_name { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc sdram_p0_max_in_collection_to_name { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[get_node_info -name [get_path_info $path -to]] == $name} {
...
proc sdram_p0_max_in_collection_from_name { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[get_node_info -name [get_path_info $path -from]] == $name} {
...
proc sdram_p0_min_in_collection_to_name2 { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc sdram_p0_min_in_collection_from_name2 { col attribute name } {
	set i 0
	set min 0
	foreach_in_collection path $col {
...
proc sdram_p0_max_in_collection_to_name2 { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[regexp $name [get_node_info -name [get_path_info $path -to]]]} {
...
proc sdram_p0_max_in_collection_from_name2 { col attribute name } {
	set i 0
	set max 0
	foreach_in_collection path $col {
		if {[regexp $name [get_node_info -name [get_path_info $path -from]]]} {
...
proc sdram_p0_get_max_clock_path_delay_through_clock_node {from through to} {
	set init 0
	set max_delay 0
...
proc sdram_p0_get_min_clock_path_delay_through_clock_node {from through to} {
	set init 0
	set min_delay 0
...
proc sdram_p0_get_model_corner {} {
...
proc sdram_p0_get_min_aiot_delay {pinname} {
...
proc sdram_p0_get_rise_aiot_delay {pinname} {
...
proc sdram_p0_get_fall_aiot_delay {pinname} {
...
proc sdram_p0_get_aiot_attr {pinname attr} {
...
proc sdram_p0_get_pll_phase_shift {output_counter_name} {
	load_package atoms
	read_atom_netlist
	set phase_shift ""
...
# ----------------------------------------------------------------
#
proc sdram_p0_get_io_standard {target_pin} {
#
...
# ----------------------------------------------------------------
	...
	set io_std [sdram_p0_get_fitter_report_pin_info $target_pin "I/O Standard" -1]
	if {$io_std == ""} {
		return "UNKNOWN"
	}
	set result ""
...
proc sdram_p0_get_rzq_pins { instname all_rzq_pins } {
...
proc sdram_p0_get_acv_read_offset { period dqs_phase dqs_period } {
...
proc sdram_p0_traverse_atom_path {atom_id atom_oport_id path} {
		...
		if {[get_atom_node_info -key type -node $atom_id] == $atom_type} {
			if {$next_direction == "end"} {
				...
				if {$atom_iport != -1} {
					...
					set result [sdram_p0_traverse_atom_path $source_atom $source_oterm [lrange $path 1 end]]
				} elseif {$atom_optional == "-optional"} {
					set result [sdram_p0_traverse_atom_path $atom_id $atom_oport_id [lrange $path 1 end]]
				}
				...
				if {$atom_oport != -1} {
					...
					foreach dest $oport_fanout {
						...
						set fanout_result_list [sdram_p0_traverse_atom_path $dest_atom -1 [lrange $path 1 end]]
...
proc sdram_p0_traverse_to_ddio_out_periphery_clock {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		...
		set pll_id_list [sdram_p0_traverse_atom_path $pin_id -1 $pin_to_pll_path]
...
proc sdram_p0_traverse_to_ddio_out_pll_clock {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
...
proc sdram_p0_traverse_to_leveling_delay_chain {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
		set leveling_id_list [sdram_p0_traverse_atom_path $pin_id -1 $pin_to_leveling_path]
...
proc sdram_p0_traverse_to_clock_phase_select {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
...
proc sdram_p0_traverse_to_dll {dqs_pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	...
	set dll_id_list [sdram_p0_traverse_atom_path $dqs_pin_id -1 $dqs_to_dll_path]
	set result ""
...
proc sdram_p0_check_hybrid_interface { inst pins_array_name mem_if_memtype } {
	...
	foreach dq_dm_pin $all_dq_dm_pins {
		set io_type [sdram_p0_get_fitter_report_pin_io_type_info $dq_dm_pin]
...
proc sdram_p0_verify_flexible_timing_assumptions { inst pins_array_name mem_if_memtype } {
	return 1
}

proc sdram_p0_verify_high_performance_timing_assumptions { inst pins_array_name mem_if_memtype } {
	upvar $pins_array_name pins
	...
	if {$num_errors == 0} {
		set msg_list [list]
		set dll_name [sdram_p0_traverse_to_dll $dqs msg_list]
		set clk_to_write_d [sdram_p0_traverse_to_ddio_out_pll_clock [lindex $all_d_list 0] msg_list]
		set clk_to_write_clock [sdram_p0_traverse_to_ddio_out_pll_clock [lindex $all_write_dqs_list 0] msg_list]
		set clk_to_ck_ckn [sdram_p0_traverse_to_ddio_out_pll_clock [lindex $pins(ck_pins) 0] msg_list]
...
proc sdram_p0_get_tccs { mem_if_memtype dqs_list period args} {
	...
	set interface_type [sdram_p0_get_io_interface_type $dqs_list]
	# The tCCS for a VHPAD interface is the same as a HPAD interface
	if {$interface_type == "VHPAD"} {
		set interface_type "HPAD"
	}
	set io_std [sdram_p0_get_io_standard [lindex $dqs_list 0]]
  	set result [list 0 0]
	if {$interface_type != "" && $interface_type != "UNKNOWN" && $io_std != "" && $io_std != "UNKNOWN"} {
		...
		if {$options(-ddr3_discrete) == 0 && $options(-write_deskew) != "none"} {
			set mode [sdram_p0_get_deskew_freq_range $tccs_params $period]
			if {$mode == [list]} {
...
proc sdram_p0_get_deskew_freq_range {timing_params period} {
	set mode [list]
...
proc sdram_p0_get_tsw { mem_if_memtype dqs_list period args} {
	global TimeQuestInfo
...
# ----------------------------------------------------------------
#
proc sdram_p0_get_fitter_report_pin_info_from_report {target_pin info_type pin_report_id} {
#
# Description: Gets the report field for the given pin in the given report
#
# ----------------------------------------------------------------
	set pin_name_column [sdram_p0_get_report_column $pin_report_id "Name"]
	set info_column [sdram_p0_get_report_column $pin_report_id $info_type]
	set result ""
...
# ----------------------------------------------------------------
#
proc sdram_p0_get_fitter_report_pin_info {target_pin info_type preferred_report_id {found_report_id_name ""}} {
#
...
# ----------------------------------------------------------------
	...
	if {$preferred_report_id == -1} {
		...
		for {set pin_report_index 0} {$pin_report_index != [llength $pin_report_list] && $result == ""} {incr pin_report_index} {
			...
			if {$pin_report_id != -1} {
				set result [sdram_p0_get_fitter_report_pin_info_from_report $target_pin $info_type $pin_report_id]
				if {$result != ""} {
					set found_report_id $pin_report_id
				}
			} else {
 				post_message -type error "sdram_p0_pin_map.tcl: Failed to find fitter report. If report timing is run after an ECO, the user must set_global_assignment -name ECO_REGENERATE_REPORT ON in sdram_p0.qsf and in sdram_p0_pin_assignment.tcl files and rerun ECO and STA"

			}
		}
	} else {
		set result [sdram_p0_get_fitter_report_pin_info_from_report $target_pin $info_type $preferred_report_id]
		if {$result != ""} {
...
# ----------------------------------------------------------------
#
proc sdram_p0_get_fitter_report_pin_io_type_info {target_pin} {
#
...
# ----------------------------------------------------------------
#
proc sdram_p0_get_io_interface_type {pin_list} {
#
...
# ----------------------------------------------------------------
	...
	foreach target_pin $pin_list {
		set io_bank [sdram_p0_get_fitter_report_pin_info $target_pin "I/O Bank" $preferred_report_id preferred_report_id]
...
# ----------------------------------------------------------------
#
proc sdram_p0_get_report_column { report_id str} {
#
...
proc sdram_p0_traverse_to_dll_id {dqs_pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set dqs_pin_id [get_atom_node_by_name -name $dqs_pin]
	set dqs_to_dll_path [list {IO_PAD fanout PADOUT} {IO_IBUF fanout O} {DQS_DELAY_CHAIN fanin DELAYCTRLIN} {DLL end DELAYCTRLOUT}]
	set dll_id_list [sdram_p0_traverse_atom_path $dqs_pin_id -1 $dqs_to_dll_path]
...
proc sdram_p0_traverse_to_dqs_delaychain_id {dqs_pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set dqs_pin_id [get_atom_node_by_name -name $dqs_pin]
	set dqs_to_delaychain_path [list {IO_PAD fanout PADOUT} {IO_IBUF fanout O} {DQS_DELAY_CHAIN atom}]
	set delaychain_id_list [sdram_p0_traverse_atom_path $dqs_pin_id -1 $dqs_to_delaychain_path]
	set delaychain_id -1
...
proc sdram_p0_get_dqs_phase_setting { dqs_pins } {
	set dqs_phase_setting 0
	set dqs0 [lindex $dqs_pins 0]
	if {$dqs0 != ""} {
		set dqs_delay_chain_id [sdram_p0_traverse_to_dqs_delaychain_id $dqs0 msg_list]
...
proc sdram_p0_get_dqs_phase { dqs_pins } {
	# Stratix V DLL Length is always 8
	set dll_length 8
	set dqs_phase_setting [ sdram_p0_get_dqs_phase_setting $dqs_pins ]

	if { $dqs_phase_setting != $::GLOBAL_sdram_p0_dqs_delay_chain_length } {
		...
		post_message -type critical_warning "   Parameter value: $::GLOBAL_sdram_p0_dqs_delay_chain_length"
...
proc sdram_p0_get_operating_conditions_number {} {
...
proc sdram_p0_get_ddr_pins { instname allpins } {
	# We need to make a local copy of the allpins associative array
	upvar allpins pins

	global ::GLOBAL_sdram_p0_dqs_group_size
	global ::GLOBAL_sdram_p0_number_of_dqs_groups
	...
	for { set i 0 } { $i < $::GLOBAL_sdram_p0_number_of_dqs_groups } { incr i } {
		...
		set dqs_local_pins [ sdram_p0_get_names_in_collection [ get_fanouts $dqs_string ] ]
		...
		set dqsn_local_pins [ sdram_p0_get_names_in_collection [ get_fanouts $dqsn_string ] ]
		...
		set dm_local_pins [ sdram_p0_get_names_in_collection [ get_fanouts $dm_string ] ]
		...
		set q_group [ list ]
		for { set j 0 } { $j < $::GLOBAL_sdram_p0_dqs_group_size } { incr j } { 
			set index [ expr $i * $::GLOBAL_sdram_p0_dqs_group_size + $j ]
			...
			set tmp_q_pins [ sdram_p0_get_names_in_collection [ get_fanouts $q_string ] ]
		...
		if { [llength $q_group] != $::GLOBAL_sdram_p0_dqs_group_size} { 
			post_sdc_message critical_warning "Could not find correct number of D pins for K pin $i. \
				Found [llength $q_group] pins. Expecting ${::GLOBAL_sdram_p0_dqs_group_size}." 
		}
	...
	foreach {pin_type pattern} $patterns { 
		set local_pins [ sdram_p0_get_names_in_collection [ get_fanouts $pattern ] ]
	...
	# CLOCK OUTPUT PLL
	set pll_ck_clock_id [sdram_p0_get_output_clock_id $pins(ck_pins) "CK Output" msg_list]
	if {$pll_ck_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "sdram_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "sdram_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(ck_pins)]"
	} else {
		set pll_ck_clock [sdram_p0_get_pll_clock_name $pll_ck_clock_id]
	}
	...
	# DQ PLL
	set pll_dq_write_clock_id [sdram_p0_get_output_clock_id [ join [ join $pins(q_groups) ]] "Write DQ CK" msg_list 20]
	if {$pll_dq_write_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "sdram_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "sdram_p0_pin_map.tcl: Failed to find PLL clock for pins [ join [ join $pins(q_groups) ]]"
	} else {
		set pll_dq_write_clock [sdram_p0_get_pll_clock_name $pll_dq_write_clock_id]
	}
	set pins(pll_dq_write_clock) $pll_dq_write_clock
	
	# DQS PLL
	set pll_write_clock_id [sdram_p0_get_output_clock_id $pins(dqs_pins) "Write CK" msg_list 20]
	if {$pll_write_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "sdram_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "sdram_p0_pin_map.tcl: Failed to find PLL clock for pins $pins(dqs_pins)"
	} else {
		set pll_write_clock [sdram_p0_get_pll_clock_name $pll_write_clock_id]
	}
	set pins(pll_write_clock) $pll_write_clock	

	# AC PLL
	set pll_ac_clock_id [sdram_p0_get_output_clock_id $pins(add_pins) "Address/Command output" msg_list]
	if {$pll_ac_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "sdram_p0_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "sdram_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(add_pins)]"
	} else {
		set pll_ac_clock [sdram_p0_get_pll_clock_name $pll_ac_clock_id]
	}
	set pins(pll_ac_clock) $pll_ac_clock

	set pll_ref_clock_id [sdram_p0_get_input_clk_id $pll_ck_clock_id]
	if {$pll_ref_clock_id == -1} {
		post_message -type error "sdram_p0_pin_map.tcl: Failed to find PLL reference clock"
	} else {
...
proc sdram_p0_initialize_ddr_db { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	global ::GLOBAL_sdram_p0_corename

	post_sdc_message info "Initializing DDR database for CORE $::GLOBAL_sdram_p0_corename"
	set instance_list [sdram_p0_get_core_instance_list $::GLOBAL_sdram_p0_corename]
	# set local_ddr_db(instance_list) $instance_list

	foreach instname $instance_list {
		post_sdc_message info "Finding port-to-pin mapping for CORE: $::GLOBAL_sdram_p0_corename INSTANCE: $instname"

		sdram_p0_get_ddr_pins $instname allpins

		sdram_p0_verify_ddr_pins allpins

		set local_ddr_db($instname) [ array get allpins ]
	}
}

proc sdram_p0_verify_ddr_pins { pins_par } {
	upvar $pins_par pins
	...
	if { $::GLOBAL_sdram_p0_number_of_dm_pins != $counted_dm_pins } {
		post_message -type critical_warning "Unexpected number of detected DM pins: $counted_dm_pins"
		post_message -type critical_warning "   expected: $::GLOBAL_sdram_p0_number_of_dm_pins"
	}
...
proc sdram_p0_get_all_instances_div_names { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc sdram_p0_get_all_instances_dqs_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db
...
proc sdram_p0_dump_all_pins { ddr_db_par } {
	upvar $ddr_db_par local_ddr_db

	set instnames [ array names local_ddr_db ]

	set filename "${::GLOBAL_sdram_p0_corename}_all_pins.txt"
	...
	puts $FH "# PIN MAP for core < $::GLOBAL_sdram_p0_corename >"
	puts $FH "#"
	puts $FH "# Generated by ${::GLOBAL_sdram_p0_corename}_pin_assignments.tcl"
	puts $FH "#"
...
proc sdram_p0_dump_static_pin_map { ddr_db_par filename } {
	upvar $ddr_db_par local_ddr_db
	...
	puts $FH "# AUTO-GENERATED static pin map for core < $::GLOBAL_sdram_p0_corename >"
	puts $FH ""
	puts $FH "proc ${::GLOBAL_sdram_p0_corename}_initialize_static_ddr_db { ddr_db_par } {"
	...
	foreach instname $instnames {
		...
		sdram_p0_static_map_expand_list $FH pins dqs_pins
		sdram_p0_static_map_expand_list $FH pins dqsn_pins

		sdram_p0_static_map_expand_list_of_list $FH pins q_groups

		puts $FH ""
		puts $FH "   set pins(all_dq_pins) \[ join \[ join \$pins(q_groups) \] \]"

		sdram_p0_static_map_expand_list $FH pins dm_pins

		sdram_p0_static_map_expand_list $FH pins ck_pins
		sdram_p0_static_map_expand_list $FH pins ckn_pins

		sdram_p0_static_map_expand_list $FH pins add_pins
		sdram_p0_static_map_expand_list $FH pins cmd_pins
		sdram_p0_static_map_expand_list $FH pins ba_pins

		puts $FH ""
		puts $FH "   set pins(ac_pins) \[ concat \$pins(add_pins) \$pins(ba_pins) \$pins(cmd_pins) \]"

		sdram_p0_static_map_expand_string $FH pins pll_ref_clock
		sdram_p0_static_map_expand_string $FH pins pll_afi_clock
		sdram_p0_static_map_expand_string $FH pins pll_ck_clock
		sdram_p0_static_map_expand_string $FH pins pll_write_clock
		sdram_p0_static_map_expand_string $FH pins pll_ac_clock
		sdram_p0_static_map_expand_string $FH pins pll_avl_clock
		...
		sdram_p0_static_map_expand_list $FH pins leveling_pins

		sdram_p0_static_map_expand_string $FH pins read_capture_ddio
		sdram_p0_static_map_expand_string $FH pins afi_reset_reg
		sdram_p0_static_map_expand_string $FH pins seq_reset_reg
		sdram_p0_static_map_expand_string $FH pins sync_reg
		sdram_p0_static_map_expand_string $FH pins fifo_wraddress_reg 
		sdram_p0_static_map_expand_string $FH pins fifo_wrdata_reg 
		sdram_p0_static_map_expand_string $FH pins fifo_rddata_reg
		sdram_p0_static_map_expand_string $FH pins valid_fifo_wrdata_reg 
		sdram_p0_static_map_expand_string $FH pins valid_fifo_rddata_reg
