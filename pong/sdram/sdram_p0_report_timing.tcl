...
source "$script_dir/sdram_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	set interface_type [sdram_p0_get_io_interface_type $all_dq_pins]
	...
	set io_std [sdram_p0_get_io_standard [lindex $dqs_pins 0]]
...
set tDCD 0.02
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_sdram_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_sdram_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		set model_corner [sdram_p0_get_model_corner]
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_sdram_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_sdram_p0_corename}-$inst)
			...
			set assumptions_valid [sdram_p0_verify_flexible_timing_assumptions $inst pins $mem_if_memtype]
			set assumptions_cache(${::GLOBAL_sdram_p0_corename}-$inst) $assumptions_valid
		}	
		...
		sdram_p0_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family scale_factors $interface_type $max_package_skew $period pins t summary MP IP board

		#######################################
		# Read Analysis

		sdram_p0_perform_flexible_read_capture_non_dqs_timing_analysis $opcs $opcname $inst $family $period $all_dq_pins pins t summary MP IP board

		#######################################
		# PHY and Address/command Analyses

		sdram_p0_perform_ac_analyses  $opcs $opcname $inst scale_factors pins t summary IP		
		sdram_p0_perform_phy_analyses $opcs $opcname $inst $inst_controller pins t summary IP

		#######################################
		# Bus Turnaround Time Analysis
		sdram_p0_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $interface_type $tJITper $tDCD $pll_steps pins t summary MP IP SSN board ISI
	...
	set summary [lsort -command sdram_p0_sort_proc $summary]

	set f -1
	if { [sdram_p0_get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_sdram_p0_corename} - Instance: $inst"
	...
	post_message -type info "Core: ${::GLOBAL_sdram_p0_corename} - Instance: $inst"
	...
	foreach summary_line $summary {
		...
		if {$su != "--"} {
			set su [ sdram_p0_round_3dp $su]
		}
		if {$hold != "--"} {
			set hold [ sdram_p0_round_3dp $hold]
		}
